...
Describe "Given: PSIISHelper Manifest" {
    Context "When: Manifest is being used" {
        $script:ThisRoot = Split-Path $PSScriptRoot -Parent
        $ManifestInfo = Test-ModuleManifest -Path ([System.IO.Path]::Combine($script:MyRoot, 'PSIISHelper.psd1'))
        ...
        It 'Then: should have the correct RootModule' {
            ...
            $RootModule | Should -Match "PSIISHelper.psm1"
        }
    }
}