BeforeDiscovery {
    ...
    Import-Module (Join-Path $ThisRoot 'PSIISHelper.psd1') -Force
}
Describe "PSIISHelper Module" {
    BeforeAll {
        $ThisRoot = Split-Path $PSScriptRoot -Parent
    }
    Context 'Module' {
    ...
    Context 'Functions' {
        ...
        #     $AllFunctions = ((Get-Content (Join-Path $script:ThisRoot 'PSIISHelper.psm1') | Select-String -Pattern $FunctionRegex).Matches.value | Foreach-Object {@($_ -Split ' ')[1]}) | Sort-Object
        ...
        It "Should be valid PowerShell code" {
            $FileContent = Get-Content (Join-Path $ThisRoot 'PSIISHelper.psm1')
            $Errors = $null
